...
onMounted(() => {
    props.purchasesHistory.forEach((file, index) => {
        purchasesHistory.value.push({
            id: index + 1,
            path: file.path,
            name: file.name,
...
const uploadFile = () => {
    ...
    Inertia.post(route('purchases.upload'), formData);
};
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="main_title_wrapper my-3">
            <h1 class="page-h1-title">仕入実績UL</h1>
        </div>
        ...
        <!--アップロード履歴ある時はこちらを表示-->
        <div v-if="purchasesHistory.length > 0">
